struct Params { style: f32, progress: f32, softness: f32 }
...
// starting at 0 would otherwise hide everything). A word crosses from
// hidden to shown as `progress` passes its own order, softened by
// `softness` into a band so words do not pop in lock-step.
fn effect(uv: vec2<f32>) -> vec4<f32> {
    let c = sample(uv);
    let order = reveal_order(uv);
    let p = clamp(params.progress / 100.0, 0.0, 1.0);
